...
class TaskSet {
  ...
  getTaskReferences() {
    return Promise.all(
      this.collectiveTasks.map((task) =>
        Promise.allSettled(task.getReferencePdfs())
      )
    );
  }
}
      ...
      for (let element of task.elements) {
        for (let lin of element.resources) {
          for (let linAmmo of lin.ammunition) {
            ...
            if (
              ammoArray.filter((item) => item["dodic"] == linAmmo["dodic"])
                .length > 0
            ) {
              ammoArray.find(
                (item) => item["dodic"] == linAmmo["dodic"]
              ).quantity += linAmmo["quantity"];
            } else {
...
class EventResource {
  ...
  constructor(options = {}) {
    const {
      lin = "SOLDIER",
      quantity = 0,
      nomenclature = "Employed by Soldiers",
      ammunition,
    } = options;
    this.ammunition = [];
    for (let option in options) {
...
class CollectiveTask {
  ...
  async init() {
    return this.getCarMetaData().then(() =>
      this?.metadata?.formats ? this.getCarData() : false
    );
  }

  async getCarMetaData() {
    this.metadata = {};
    return carSearch(this.id).then((carMetadata) => {
  ...
  async getCarData() {
    let metaURL = this["metadata"]["formats"].find(
      (item) => item["path"] == "metadata.json"
    )?.["link"]?.["href"];
    //console.log(metaURL)
  ...
  getUrlPdf() {
    return this["metadata"]["formats"].find(
      (item) => item["path"] == "report.pdf"
    )["link"]["href"];
  }
    ...
    if (this["references"]?.length < 1) {
      ...
      return (
        this["references"].map((reference) =>
          carSearch(reference["id"]).then((meta) => getPdfLink(meta))
        ) || undefined
      );
    }
  }
...
async function carSearch(docId) {
  ...
  return fetch(`https://rdl.train.army.mil/catalog-ws/api/catalogitems.json?current=true
            &search_terms=${encodedDocId}&page=1&pagesize=20&field_list=*`).then(
    (res) =>
      toJSON(res.body)
        .then((jsonBody) => {
        ...
        .then((firstReturn) => {
          if (
            firstReturn["identifier"] == docId ||
            firstReturn["alternateid"] == docId
          ) {
            return firstReturn;
          } else {
            console.log(
              `SOFT-ERROR First return did not match requested document, wanted ${docId}, received ${
                firstReturn["identifier"] ||
                firstReturn["alternateid"] ||
                firstReturn["title"]
              }`
            );
            return undefined;
...
function getPdfLink(metadata) {
  if (
    metadata["formats"].find((item) => item["path"] == "report.pdf")?.[
      "link"
    ]?.["href"]
  ) {
    return metadata["formats"].find((item) => item["path"] == "report.pdf")[
      "link"
    ]["href"];
  } else {
